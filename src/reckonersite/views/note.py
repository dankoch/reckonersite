...
from reckonersite.client.noteclient import client_post_reckoning_favorite, \
                                           ...
                                           client_post_content_favorite, \
                                           client_post_content_flag, \
                                           client_post_content_comment_favorite, \
                                           client_post_content_comment_flag
...
###############################################################################################
## The endpoint responsible for posting a favorite to content
#  (as used primarily for AJAX calls).
###############################################################################################
...
###############################################################################################
## The endpoint responsible for posting a flag to content
#  (as used primarily for AJAX calls).
###############################################################################################

def post_content_flag(request, id = None):
    ...
    try:
        if request.method == 'POST':
            if (id):
                flag = Flag(user_id = request.user.reckoner_id)
                service_response = client_post_content_flag(flag, id, request.user.session_id)
        ...
        logger.error("Exception when flagging content:") 
        logger.error(traceback.print_exc(8))    
        site_response = AjaxServiceResponse(success=False,
                                            message="whoops", 
                                            message_description='No go. Try again later.')
...
###############################################################################################
## The endpoint responsible for posting a favorite to a content comment
#  (as used primarily for AJAX calls).
###############################################################################################
